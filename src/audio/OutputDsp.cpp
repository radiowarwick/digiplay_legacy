...
OutputDsp::OutputDsp(string channel) {
	...
	string soName = "OutputSO_" + channel.substr(0, o) + ".so";
	deviceName = channel.substr(o+1);
	cout << "Loading: " << soName << endl;
	// Try $DPSDIR/modules first, then LD_LIBRARY_PATH search..
	string tmpName = DPSDIR "/modules/" + soName;
	...
	OutputSO_Entry entry = (OutputSO_Entry)dlsym(dlHandle, OUTPUT_SO_SYM);
	if (!entry) {
		cerr << "No entry point function in module: " << soName << endl;
		dlclose(dlHandle);
		throw 0;
	}
	cout << "Connecting: " << deviceName << endl;
...
OutputDsp::~OutputDsp() {
	if (pImpl) {
		cout << "Unloading: " << deviceName << endl;
		delete pImpl;
	}
	if (dlHandle)
		dlclose(dlHandle);
