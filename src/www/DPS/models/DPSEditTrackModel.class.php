		...
		if(is_numeric($trackID) && $trackID != "" && $trackID != "*") {
			...
			if($this->fieldData['album'] != '') {
				$sql = "SELECT id FROM albums where name = '" .
					pg_escape_string($this->fieldData['album']) . "'";
				...
				for($i=0;$i<=$_POST['artistnum'];$i++) {
					...
					if($this->fieldData[$artistnum] != "") {
						$sql = "SELECT * FROM artists where name = '" .
							pg_escape_string($this->fieldData[$artistnum]) . "'";
						...
						if($i == $this->fieldData['artistnum']) {
							...
							if($newID != $this->fieldData[$artistnumid] 
								&& is_numeric($this->fieldData[$artistnumid])) {
								$aaValues['artistid'] = $newID;
								$aaWhere = "audioid = " . $trackID . " 
									AND artistid = " . pg_escape_string($this->fieldData[$artistnumid]);
								$db->update('audioartists', $aaValues, $aaWhere, true);
							}
						}
					} elseif ($i != $this->fieldData['artistnum']) {
						$aaWhere = "audioid = " . $trackID . " AND artistid = " 
							. pg_escape_string($this->fieldData[$artistnumid]);
			...
			if(is_numeric($this->fieldData['keywordnum'])) {
				for($i=0;$i<=$_POST['keywordnum'];$i++) {
					...
					if($this->fieldData[$keywordnum] != "") {
						$sql = "SELECT * FROM keywords where name = '" .
							pg_escape_string($this->fieldData[$keywordnum]) . "'";
						...
						if($i == $this->fieldData['keywordnum']) {
							...
							if($newID != $this->fieldData[$keywordnumid]
								&& is_numeric($this->fieldData[$keywordnumid])) {
								$akValues['keywordid'] = $newID;
								$akWhere = "audioid = " . $trackID . "
									AND keywordid = " . pg_escape_string($this->fieldData[$keywordnumid]);
								$db->update('audiokeywords',$akWhere, $akValues, true);
							}
						}
					} elseif ($i != $this->fieldData['keywordnum']) {
						$akWhere = "audioid = " . $trackID . " AND keywordid = "
							. pg_escape_string($this->fieldData[$keywordnumid]);
