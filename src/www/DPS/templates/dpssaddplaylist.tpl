{include file="rfile:dpssuetopframe.tpl" title="RaW Digiplay Manager" nexttrack=$VIEWER.sueNextTrack suesearchid=$VIEWER.templateID Access_EditSue=$VIEWER.Access_EditSue Access_SueStats=$VIEWER.Access_SueStats Access_ViewSue=$VIEWER.Access_ViewSue}
	    ...
	    <div class="sueplaylistInfo1">
	      ...
	      Sort by artist
	      {else}
	      {templateLink id="62" text="Sort by artist" _dpsSearchLetter=$VIEWER.searchLetter _dpsSortType="artist" _dpsSearchType=$VIEWER.searchType _dpsSearchVal=$VIEWER.searchValue}
	      {/if}
	      | 
	      {if $VIEWER.sortType == "title"}
	      Sort by title
	      {else}
	      {templateLink id="62" text="Sort by title" _dpsSearchLetter=$VIEWER.searchLetter _dpsSortType="title" _dpsSearchType=$VIEWER.searchType _dpsSearchVal=$VIEWER.searchValue}
	      {/if}
	    </div>
	    <div class="sueplaylistInfo2">
	      Click a track's title for more information
	    </div>
	    <form name="dpsSueAddPlaylistForm" method="post" action="{$CONFIG.general.siteRoot}">
