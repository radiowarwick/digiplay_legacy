{include file="rfile:dpsstudiotopframe.tpl" title="Digiplay Manager" studio=$VIEWER.studioAccess access_playlist=$VIEWER.access_playlist}
	...
	<div class='awwallinfoR'>
		{templateLink id="23" text="<<< return to audiowall profiles"} 
		{if $VIEWER.editperm == 't'}
		<br>{templateLink id="25" text="<<< edit audiowall" _awset=$VIEWER.awsetID _page=$VIEWER.awwall.page} 
		{/if}
	</div>
	<br style='clear: both;'>
		...
		<div class='awwallpagenum'>
			...
			{foreach from=$VIEWER.pagelink item=pages}
				{if $pages.active == 't'}
					<b>{$pages.num}</b>
				{else}
					{templateLink id="67" text=$pages.num _awset=$VIEWER.awsetID _page=$pages.id} 
				{/if}
			{/foreach}
		</div>
