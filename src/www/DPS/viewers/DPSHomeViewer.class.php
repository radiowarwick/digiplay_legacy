...
class DPSHomeViewer extends Viewer {
  ...
  protected function setupTemplate() {
    ...
    $sql = "select shows.* from shows , schedule where(shows.id = schedule.showid and '$current_time' > schedule.start and '$current_time' <= schedule.finish);";
    $showC = $db_web->getRow($sql);
    if(!$showC) {
      $sql = "select * from shows where id = " . pg_escape_string($cfg['DPS']['defaultShowID']);
      $showC = $db_web->getRow($sql);
    }
    
    //Show on next
    $sql = "select shows.* from shows , schedule where(shows.id = schedule.showid and '$current_time' < schedule.start) ORDER BY schedule.start ASC LIMIT 1;";
    ...
    //Show just on
    $sql = "select shows.* from shows , schedule where(shows.id = schedule.showid and '$current_time' > schedule.finish) ORDER BY schedule.finish DESC LIMIT 1;";
