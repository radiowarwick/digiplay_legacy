...
class DPSShowPlanSelectMusicViewer extends Viewer {
	...
	protected function setupTemplate() {
		...
		if(is_numeric($itemID) && isset($itemID)) {	
			...
			FROM (
				SELECT showplanusers.permissions FROM showplanusers, showitems 
				WHERE showitems.showplanid = showplanusers.showplanid AND 
					...
					if($item['id'] == $itemID) {
						...
						if($item['audio'] != '') {
							$sql = "SELECT audio.title as title, audiotypes.name as type, 
								...
								WHERE audio.type = audiotypes.id and audio.id = " . $item['audio'];
							$stuff = $db->getRow($sql);
