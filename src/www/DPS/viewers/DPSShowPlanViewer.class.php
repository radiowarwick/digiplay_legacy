...
class DPSShowPlanViewer extends Viewer {
  ...
  protected function setupTemplate() {
    ...
    $date = time();
		$show_query = "SELECT DISTINCT showplans.* FROM showplans, showplanusers where 
										showplans.id = showplanusers.showplanid AND 
										showplanusers.userid = " . $userID . " AND 
										showplans.showdate >= " . $date . " AND 
										showplanusers.permissions & B'" . $cfg['DPS']['fileR'] . "' = '" . $cfg['DPS']['fileR'] . "' 
										UNION(SELECT showplans.* FROM showplans, showplangroups, usersgroups where 
										showplans.id = showplangroups.showplanid AND 
										usersgroups.userid = " . $userID . " AND 
										usersgroups.groupid = showplangroups.groupid AND	 
										showplans.showdate >= " . $date . " AND
										showplangroups.permissions & B'" . $cfg['DPS']['fileR'] . "' = '" . $cfg['DPS']['fileR'] . "')  
										ORDER BY name asc";
