...
//    $db_web = Database::getInstance($cfg['DPS']['dsn_web']);
	...
	$this->assign('cart' . ($i+1), $tcart);
      ...
      $sql = "select count(*) from cartsetsusers where userid = " . $cfg['DPS']['systemUserID'] . " AND cartsetid = $cartset AND (permissions = 'w' OR permissions = 'rw' OR permissions = 'o')";
      $check = $db->getOne($sql);
      if($check > 0) {
	$this->assign('editperm','t');
      }else {
	$sql = "select count(*) from cartsetsgroups, groupmembers where groupmembers.userid = " . $cfg['DPS']['systemUserID'] . " AND cartsetsgroups.groupid = groupmembers.groupid AND cartsetsgroups.cartsetid = $cartset AND (cartsetsgroups.permissions = 'w' OR cartsetsgroups.permissions = 'rw')";
	$check = $db->getOne($sql);
	if($check > 0) {
	  $this->assign('editperm','t');
	}
