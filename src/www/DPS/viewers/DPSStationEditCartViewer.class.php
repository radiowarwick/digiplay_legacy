...
class DPSStationEditCartViewer extends Viewer {
  ...
  protected function setupTemplate() {
	...
	if($check > 0) {
      ...
      if($flag) {
    // cartwalls
      $sql = "select cartsaudio.id as id, cartsaudio.text as text,cartsaudio.audio as audioID, cartwalls.cartset as cartset, 
	    cartsaudio.style as styleID,cartsaudio.cart as cart from cartsaudio, cartwalls where cartsaudio.cartwall = cartwalls.id and cartsaudio.id = " . $cartID;
      $cart = $db->getRow($sql);
    ...
    if($flag) {
      ...
      if($cartID == "New") {
	$sql = "select count(*) from cartsetsusers, cartwalls where cartwalls.cartset = cartsetsusers.cartsetid and cartwalls.id = " . $cartwallID . " and cartsetsusers.userid = $userID and cartsetsusers.permissions = 'o'";
	$owner = $db->getOne($sql);
      } else {
	$sql = "select count(*) from cartsetsusers where cartsetid = " . $cart['cartset'] . " and cartsetsusers.userid = $userID and permissions = 'o'";
	$owner = $db->getOne($sql);
      }
      if($owner > 0) {
