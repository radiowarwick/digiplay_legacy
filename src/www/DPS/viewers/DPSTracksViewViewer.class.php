...
class DPSTracksViewViewer extends Viewer {
  ...
  protected function setupTemplate() {
    ...
    foreach($trackIDs as $trackID) {
      $sql = "SELECT audio.*, albums.name as album FROM audio, albums where audio.music_album = albums.id and audio.id = " . pg_escape_string($trackID);
      $trackDetails = $db->getRow($sql);
      $sql = "SELECT DISTINCT artists.name as name FROM artists, audioartists WHERE audioartists.audio = " . pg_escape_string($trackID) . " AND audioartists.artist = artists.id";
      $trackDetails['artist'] = $db->getColumn($sql);
    
      $sql = "SELECT DISTINCT keywords.name as name FROM keywords, audiokeywords WHERE audiokeywords.track = " . pg_escape_string($trackID) . " AND audiokeywords.keyword = keywords.id";
    ...
    $this->assign('Access_CommentTrack',AuthUtil::getDetailedUserrealmAccess(array(58,59,77), $userID));
		$this->assign('Access_EditTrack',AuthUtil::getDetailedUserrealmAccess(array(58,59,61), $userID));
