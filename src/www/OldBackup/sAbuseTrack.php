...
(useraccess.privilage=3 or useraccess.privilage=6)";
...
?>
<h1>Censor Unsuitable Track</h1><hr>
...
<hr>
	    ...
	    <a href="/dps/sAbuseTrack.php?section=0">0</a>
	    <a href="/dps/sAbuseTrack.php?section=1">1</a>
	    <a href="/dps/sAbuseTrack.php?section=2">2</a>

	    <a href="/dps/sAbuseTrack.php?section=3">3</a>
	    <a href="/dps/sAbuseTrack.php?section=4">4</a>
	    <a href="/dps/sAbuseTrack.php?section=5">5</a>
	    <a href="/dps/sAbuseTrack.php?section=6">6</a>
	    <a href="/dps/sAbuseTrack.php?section=7">7</a>
	    <a href="/dps/sAbuseTrack.php?section=8">8</a>

	    <a href="/dps/sAbuseTrack.php?section=9">9</a>
	    <a href="/dps/sAbuseTrack.php?section=a">A</a>
	    <a href="/dps/sAbuseTrack.php?section=b">B</a>
	    <a href="/dps/sAbuseTrack.php?section=c">C</a>
	    <a href="/dps/sAbuseTrack.php?section=d">D</a>
	    <a href="/dps/sAbuseTrack.php?section=e">E</a>

	    <a href="/dps/sAbuseTrack.php?section=f">F</a>
	    <a href="/dps/sAbuseTrack.php?section=g">G</a>
	    <a href="/dps/sAbuseTrack.php?section=h">H</a>
	    <a href="/dps/sAbuseTrack.php?section=i">I</a>
	    <a href="/dps/sAbuseTrack.php?section=j">J</a>
	    <a href="/dps/sAbuseTrack.php?section=k">K</a>

	    <a href="/dps/sAbuseTrack.php?section=l">L</a>
	    <a href="/dps/sAbuseTrack.php?section=m">M</a>
	    <a href="/dps/sAbuseTrack.php?section=n">N</a>
	    <a href="/dps/sAbuseTrack.php?section=o">O</a>
	    <a href="/dps/sAbuseTrack.php?section=p">P</a>
	    <a href="/dps/sAbuseTrack.php?section=q">Q</a>

	    <a href="/dps/sAbuseTrack.php?section=r">R</a>
	    <a href="/dps/sAbuseTrack.php?section=s">S</a>
	    <a href="/dps/sAbuseTrack.php?section=t">T</a>
	    <a href="/dps/sAbuseTrack.php?section=u">U</a>
	    <a href="/dps/sAbuseTrack.php?section=v">V</a>
	    <a href="/dps/sAbuseTrack.php?section=w">W</a>

	    <a href="/dps/sAbuseTrack.php?section=x">X</a>
	    <a href="/dps/sAbuseTrack.php?section=y">Y</a>
	    <a href="/dps/sAbuseTrack.php?section=z">Z</a>
	    <p>
		<!-- search form -->
		<FORM METHOD=POST ACTION='/dps/sAbuseTrack.php?search=1'>
		  Find songs where the 
		  <SELECT NAME='field'>
		  <OPTION VALUE='artists.name'>Artist</OPTION>
		  <OPTION VALUE='audio.title'>Track</OPTION>
                  <OPTION VALUE='audio.flagged'>Flagged</OPTION>
		  </SELECT>
		  contains
		  <INPUT TYPE='text' NAME='terms'>
		  <INPUT TYPE='submit' VALUE='Search'>
		  		</FORM>To find all flagged tracks just select Flagged and click search.
	    </p>
<?
...
if(isset($change)) { //if they are changing something
        ...
        for ($i=1; $i<=$max; $i++) {
          if ((!isset($_POST['song'.$i]) && $_POST['original'.$i] == 'on')
            ...
            if($_POST['song'.$i] == 'on') {
              $sql = "UPDATE audio SET censor = 't', flagged = 'f', sustainer = 'f' WHERE id = " . pg_escape_string($_POST['id'.$i]);
              $update = pg_query($dbconn1,$sql);
            } else {
              $sql = "UPDATE audio SET censor = 'f', flagged = 'f' WHERE id = " . pg_escape_string($_POST['id'.$i]);
              $update = pg_query($dbconn1,$sql);
            }
            $track = pg_escape_string($_POST['id'.$i]);
            $query = "SELECT title, censor FROM audio WHERE audio.id = '$track'";
            $result = pg_query($dbconn1, $query);
            $count = pg_fetch_row($result, 0);
            if ($count[1] == 'f') {
              echo "<font color='red'>Track '" . $count[0] . "' un-censored</font><br />";
            } else {
              echo "<font color='red'>Track '" . $count[0] ."' censored</font><br />";
            } //end $count etc etc
...
echo "<b>Playlist Successfully Updated</b><br><p>";
		...
		if ($field=='audio.flagged') {
                  //SQL Query to get no. of search results
                  $count_query = pg_query("SELECT COUNT(audio.id) FROM artists,audio,audioartists WHERE artists.id = audioartists.artist AND audio.id = audioartists.audio AND $field = 't'");
                  //SQL Query to get search results
                  $query = pg_query("SELECT DISTINCT artists.name AS artist,audio.title AS title,audio.id AS id, audio.flagged, audio.censor WHERE artists.id = audioartists.artist AND audio.id = audioartists.audio AND $field = 't' ORDER BY audio.title LIMIT 200 OFFSET ".$offset*200);
                } else if ($search) {
                  //SQL Query to get no. of search results
                  ...
                  if ($search) {
                    ...
                    for ($loop = 1; $loop <= $pagecount; $loop++) {
                      if ($loop-1 == $offset) {
                        ...
                        //if not current page
                        $pagestr .= "&nbsp;<A HREF='/dps/sAbuseTrack.php?section=$section&offset=";
                        $pagestr .= $loop-1;
                        $pagestr .= "'>";
                        $pagestr .= (($loop-1)*200)+1;
                        $pagestr .= " - ";
                ...
                <form method=POST action='/dps/sAbuseTrack.php?change=1'>
                                ...
                                <tr>
                                  <td style='text-align:center;'>
                                    ...
                                    if ($tracks[4] == 't') {
                                            ...
                                            echo "<input type='checkbox' name='song$total'>(Flagged)
                                                  <input type=hidden name='id$total' value='$tracks[id]'>
